/* ============================================
   ONE PIECE SEALED PRODUCTS PAGE
   ============================================
   
   Página para visualizar productos sellados de One Piece
...
export async function render(container) {
    console.log('📦 Renderizando One Piece Sellado...');
    
    container.innerHTML = getOnePieceSealedHTML();
    await initOnePieceSealed();
    
    console.log('✅ One Piece Sellado renderizado');
}

/**
 * Genera el HTML
 */
function getOnePieceSealedHTML() {
    return `
        <div class="onepiece-sealed-container">
            <!-- Header -->
            <div class="page-header">
                <div>
                    <h1 class="page-title">
                        <i class="bi bi-flag"></i>
                        One Piece: Productos Sellados
                    </h1>
                    <p class="page-subtitle">
                        Gestión de productos sellados de One Piece Card Game
                    </p>
                </div>
 ...
 */
async function initOnePieceSealed() {
    // Event listeners
...
async function loadProducts() {
    try {
        showLoader();
        console.log('🔄 Iniciando carga de One Piece Sealed...');

        const response = await api.get('/products/sealed/onepiece', { limit: 1000 });
        
        console.log('✅ Respuesta recibida:', response);

        if (!response) {
            console.error('❌ Respuesta vacía');
            showError('❌ Error: Respuesta vacía');
            return;
        }
        ...
        console.log(`✅ Cargados ${allProducts.length}`);
        currentPage = 1;
        renderProducts();
    ...
    if (productsToShow.length === 0) {
        grid.innerHTML = `
            <div class="empty-state">
                ...
                <p>No se encontraron productos sellados de One Piece.</p>
            </div>
        `;
