/* ============================================
   POKEMON SEALED PRODUCTS PAGE
   ============================================
   
   Página para visualizar productos sellados de Pokémon
   (Booster Box, ETB, Collections, etc.)
...
export async function render(container) {
    console.log('📦 Renderizando Pokemon Sellado...');
    
    container.innerHTML = getPokemonSealedHTML();
    await initPokemonSealed();
    
    console.log('✅ Pokemon Sellado renderizado');
}

/**
 * Genera el HTML
 */
function getPokemonSealedHTML() {
    return `
        <div class="pokemon-sealed-container">
            <!-- Header -->
            <div class="page-header">
                <div>
                    ...
                    <p class="page-subtitle">
                        Gestión de productos sellados de Pokémon TCG
                    </p>
                </div>
 ...
 */
async function initPokemonSealed() {
    // Event listeners
...
async function loadProducts() {
    try {
        showLoader();
        console.log('🔄 Iniciando carga de Pokemon Sealed...');

        const response = await api.get('/products/sealed/pokemon', { limit: 1000 });
        ...
        const errorMsg = `
            <div style="text-align: center; padding: 2rem;">
                ...
                <p>Backend URL: <code>http://127.0.0.1:3000/api</code></p>
                <p style="font-size: 0.9em; color: #666;">
                    Verifica que el backend esté corriendo con: <code>npm start</code>
                </p>
            </div>
        `;
        showError(errorMsg);
    ...
    if (productsToShow.length === 0) {
        grid.innerHTML = `
            <div class="empty-state">
                ...
                <p>No se encontraron productos sellados de Pokémon.</p>
            </div>
        `;
